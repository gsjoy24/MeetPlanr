...
const BarChartPage = () => {
  ...
  console.log((allSchedule?.filter(scheduled => (
    ((new Date(scheduled.timeStamp).getDate()) == today.getDate()))))?.length);
  const data = {labels,
    datasets: [
      {
        label: 'Schedule',
        data: dateArray.map((date) => {
          const quantity = allSchedule?.filter(scheduled => (
            (new Date(scheduled.timeStamp).getDate()) == (new Date(date).getDate())
            ));
      ...
      },
      {
        label: 'Users',
        data: dateArray.map((date) => {
          const quantity = allUsers?.filter(user => (
            (new Date(user.timestamp).getDate()) == (new Date(date).getDate())
            ));
          return quantity?.length;
        }),
      ...
      },
      {
        label: 'Blogs',
        data: dateArray.map((date) => {
          const quantity = allBlogs?.filter(blog => (
            (new Date(blog.timestamp).getDate()) == (new Date(date).getDate())
            ));
          return quantity?.length;
        }),
