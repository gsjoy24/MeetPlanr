...
export const POST = async(request)=>  {
    if (request.method === "POST") {
      ...
      const {inviteeName,inviteeEmail,eventName,hostEmail,scheduleDate,method,hostName,} = body || {};
      const msg = {
        to: `${hostEmail}`, // Change to your recipient
        from: 'meetplanr@gmail.com', // Change to your verified sender
        subject: `${hostName} a new event has been scheduled in MeetPlanr`,
        ...
        <p>Hi ${hostName},</p>
        <p  style=" margin: 0;">A new event scheduled has been confirmed with ${inviteeName}.</p>
        <h4 style="margin-bottom: 5px;">Event Name:</h4>
        <p style=" margin: 0;">${eventName}</p>
        <h4 style="margin-bottom: 5px;">Invitee Email:</h4>
        <p style=" margin: 0;">${inviteeEmail}</p>
        ...
        <p>${method}</p>
        <a href="#" style="margin-top: 30px; display: inline-block;">More Information in <b>MeetPlanr</b></a>
    </div>`,
      }
      try {
        sgMail.send(msg)
